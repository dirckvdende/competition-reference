...
-pipe -lm"
\end{minted}
{\color{gray}\texttt{(\competitionname)}}
...
\begin{itemize}
    \item \textsc{get:} Get the sum (or other operation) of the elements on an interval $[a, b]$. $\mathcal O(\log n)$
    \item \textsc{update:} Update an entry in list that the tree represents. $\mathcal O(\log n)$
\end{itemize}
\vspace{-0.3cm}
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[main/.style = {draw, rectangle, align=center}, node distance=2cm]
        \node[main] (0) at (0, 0) {\texttt{j}};
        \node[main] (1) at (-1, -1) {\texttt{j << 1}};
        \node[main] (2) at (1, -1) {\texttt{j << 1 | 1}};
...
\end{figure}
\vspace{-0.3cm}
The implementation uses nodes which store the sum of all of the leaves below it.
