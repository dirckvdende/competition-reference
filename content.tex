...
The inverse of an integer $x$ modulo $n$ can be found using the extended Euclidean algorithm. An inverse exists if and only if $\text{gcd}(x, n) = 1$. Finding a solution $x$ to the system
\begin{align*}
...
\end{align*}
Then we have $x = x'\cdot \frac{b}{\text{gcd}(a, n)}$. If the given fraction is not an integer, the inverse does not exist.
...
\begin{align*}
    A = \frac12\sum_{k=0}^{n-1} (x_ky_{k+1} - x_{k+1}y_k).
\end{align*}
...
\end{align*}
where $\tilde A_{ij}$ is the matrix $A$ with row $i$ and column $j$ removed.
\code{src/math/detfast.cc}
...
\subsection{Maximum bipartite matching $\mathcal O(V(V + E)^2)$}
...
A version of Quicksort (combined with linear sort) is implemented in \texttt{C++}, use \texttt{sort(all(list), lt)}.
